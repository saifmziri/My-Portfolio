    ...
    <>
      ...
      */}
      <section
        ref={containerRef}
        id="projects"
        className="relative bg-zinc-950/40 border-t border-white/10"
        style={sectionStyle}
      >
        ...
        >
          <div className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-6 lg:py-8">
            ...
            <div className="flex items-center justify-between gap-4 mb-4 sm:mb-6 lg:mb-10">
              ...
              <div className="flex items-center gap-2 sm:gap-3 font-mono text-xs text-zinc-500">
                <button
                  onClick={() => scrollToProject(Math.max(0, activeProjectIndex - 1))}
                  disabled={activeProjectIndex === 0}
                  className="p-1.5 sm:p-2 rounded-full border border-white/10 hover:text-white disabled:opacity-30 disabled:pointer-events-none transition-colors"
            ...
            <div className="hidden lg:grid grid-cols-12 gap-8 items-center" style={{ minHeight: '520px' }}>
              ...
              <div className="col-span-5 flex flex-col justify-between h-full space-y-5">
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div>
                      <h3 className="text-3xl xl:text-4xl font-bold text-white tracking-tight mb-2">
                        {activeProject.title}
                      </h3>
              ...
              {/* Image column */}
              <div className="col-span-7 relative rounded-2xl border border-white/10 overflow-hidden bg-zinc-900 shadow-2xl group" style={{ height: '500px' }}>
                <AnimatePresence mode="wait">
                  <motion.div
                    key={activeProject.id}
            ...
            <div className="lg:hidden">
              <AnimatePresence mode="wait">
                ...
                >
                  <SpotlightCard className="p-4 sm:p-5 space-y-3 rounded-2xl border border-white/10 bg-zinc-900/90 shadow-2xl backdrop-blur-sm">
                    {/* Top bar */}
                    ...
                    <div className="relative rounded-xl overflow-hidden border border-white/10 bg-zinc-950">
                      ...
                      <button
                        onClick={() => setSelectedProject(activeProject)}
                        className="absolute bottom-2.5 right-2.5 px-3 py-1.5 rounded-full bg-zinc-950/85 backdrop-blur-md border border-white/15 text-[11px] font-mono text-white flex items-center gap-1.5 shadow-lg active:scale-95 transition-transform"
                      >
                    ...
                    <div className="grid grid-cols-3 gap-1.5 py-2 border-y border-white/10">
                      {activeProject.metrics.slice(0, 3).map((m, idx) => (
                        <div key={idx} className="space-y-0.5 text-center">
                          <span className="block text-[9px] font-mono text-zinc-500 uppercase truncate">
                            {m.label}
                          </span>
                          <span className="block text-[10px] font-bold text-white font-mono truncate">
                            {m.value}
                          </span>
                        </div>
                      ))}
                    ...
                    <div className="flex flex-wrap gap-1">
                      {activeProject.technologies.slice(0, 5).map((tech, idx) => (
                        <span key={idx} className="px-2 py-0.5 text-[10px] font-mono rounded bg-white/5 border border-white/10 text-zinc-300">
                          {tech}
                        </span>
                      ))}
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-white/10">
                      <button
                        onClick={() => setSelectedProject(activeProject)}
                        className="px-4 py-2 rounded-full bg-indigo-600 active:bg-indigo-500 text-white text-xs font-mono font-medium flex items-center gap-1.5 shadow-md shadow-indigo-600/20"
                      >
                        <span>View Case Study</span>
                        <ArrowUpRight className="w-3.5 h-3.5" />
                      </button>
                      ...
                      <div className="flex items-center gap-1.5">
                        {activeProject.githubFrontend && (
                          <a href={activeProject.githubFrontend} target="_blank" rel="noopener noreferrer"
                            className="p-2 rounded-full border border-cyan-500/30 bg-cyan-950/40 text-cyan-400 active:text-white"
                        ...
                        {activeProject.githubBackend && (
                          <a href={activeProject.githubBackend} target="_blank" rel="noopener noreferrer"
                            className="p-2 rounded-full border border-indigo-500/30 bg-indigo-950/40 text-indigo-400 active:text-white"
                        ...
                        {activeProject.github && !activeProject.githubFrontend && !activeProject.githubBackend && (
                          <a href={activeProject.github} target="_blank" rel="noopener noreferrer"
                            className="p-2 rounded-full border border-white/10 text-zinc-400 active:text-white"
                            aria-label="GitHub Repository">
                            <GithubIcon className="w-3.5 h-3.5" />
                          </a>
                        )}
                        {activeProject.linkedinPost && (
                          <a href={activeProject.linkedinPost} target="_blank" rel="noopener noreferrer"
                            className="p-2 rounded-full border border-blue-500/30 bg-blue-950/40 text-blue-400 active:text-white"
                        ...
                        {activeProject.instagramPost && (
                          <a href={activeProject.instagramPost} target="_blank" rel="noopener noreferrer"
                            className="p-2 rounded-full border border-pink-500/30 bg-pink-950/40 text-pink-400 active:text-white"
